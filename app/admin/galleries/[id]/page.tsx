...
export default function EditGallery({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        if (!galleryId) {
            toast.error("ID Galerie Invalid");
        ...
        const fetchGallery = async () => {
            try {
                ...
                if (error) {
                    toast.error("Eroare la încărcarea galeriei: " + error.message);
                    router.push("/admin/galleries");
                    return;
                }
    ...
    const handleDelete = async () => {
        if (!confirm("Ești sigur că vrei să ștergi această galerie? Acțiunea este ireversibilă.")) return;
        setDeleting(true);
        try {
            const { error } = await supabase.from('galleries').delete().eq('id', galleryId);
            if (error) throw error;
            toast.success("Galerie ștearsă!");
            router.push("/admin/galleries");
            router.refresh();
        } catch (err: any) {
    ...
    if (fetching) {
        return <div className="text-white text-center py-20"><T>Se încarcă...</T></div>;
    }

    return (
        <div className="max-w-5xl mx-auto pb-20">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold uppercase tracking-tighter text-white"><T>Editare Galerie</T></h1>
                <div className="flex gap-4">
                    <button
                        onClick={handleDelete}
                        disabled={deleting}
                        className="bg-zinc-800 px-4 py-2.5 rounded text-red-500 font-bold uppercase tracking-widest hover:bg-red-900/50 hover:text-red-400 transition-all flex items-center gap-2"
                    >
                    ...
                    >
                        {loading ? <T>Se salvează...</T> : <T>Salvează Modificările</T>}
                    </button>
                </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg space-y-4">
                    <div>
                        <label className="block text-sm font-semibold text-zinc-400 uppercase tracking-widest mb-2"><T>Adaugă Fișiere Media (Poze / Video)</T></label>
                        <div className="flex gap-4 items-center">
                            <input
                                type="file"
                                accept="image/*,video/*"
